...
function updateGameDoc(gameId, player, data = {}, onError) {
  // console.log('updating game', { gameId, player, data })
...
export function startRemoteGame({ gameId, /* player, */ onRemotePlayerMove, onRemotePlayerDisconnected }) {
  ...
  let lastMove, player, remotePlayerConnected, pingInterval  
  const cleanupOnSnapshot = firestore.onSnapshot(firestore.doc(firebase.db, COLLECTION_NAME, gameId), (snapshot) => {
    ...
    if (snapshot && snapshot.data()) {
      const { status, moves, player_x_updated, player_o_updated } = snapshot.data()
      let remotePlayerUpdated = (player === 'x' && player_o_updated) || (player === 'o' && player_x_updated)

      if (!player) {
        // joining initially: whatever player was not updated is this player
        // note that player o is invited and so should be the first one in

        if (!player_o_updated) {
          player = 'o'
          remotePlayerUpdated = player_x_updated
        } else if (!player_x_updated) {
          player = 'x'
          remotePlayerUpdated = player_o_updated
        }
      ...
      // mark remote player as connected in order to properly later register as disconnected later
      if (!remotePlayerConnected && remotePlayerUpdated) remotePlayerConnected = true
      ...
      } else if (remotePlayerConnected && [STATUS_INIT, STATUS_PLAYING].includes(status) && calcPlayerStatus(remotePlayerUpdated) !== PLAYER_STATUS_CURRENT) {
